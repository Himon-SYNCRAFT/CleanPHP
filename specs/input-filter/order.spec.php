	...
	describe('->isValid()', function() {
		...
		it('should require order numbers be 13 chars long', function() {
			...
			foreach ($scenarios as $scenario) {
				$this->inputFilter = new OrderInputFilter();
				$this->inputFilter
					->setData(['orderNumber' => $scenario['value']])
					->isValid();

				$messages = $this->inputFilter->getMessages()['orderNumber'] ?? null;
		...
		it('should require a total', function() {
			...
			$errors = ['isEmpty' => 'Value is required and can\'t be empty'];
			$messages = $this->inputFilter->getMessages()['total'];

			expect($isValid)->to->equal(false);
			expect($messages)->to->equal($errors);
		});
